...
def test_model(cfg):
    ...
    classifier_path = os.path.join(cfg['output_dir'], cfg['timestamp'], f"classifier_best_{cfg['timestamp']}.pth")
    ...
    # 類別內變異分析（optional）
    if cfg.get('tsne', True):
        var_path = os.path.join(cfg['output_dir'], cfg['timestamp'], 'intra_class_variance.png')
    ...
    # kNN Confusion Matrix
        if cfg.get("tsne", True):  # 使用相同的 flag 控制
            knn_cm_path = os.path.join(cfg["output_dir"], cfg["timestamp"], "knn_confusion_matrix.png")
            plot_knn_confusion_matrix(torch.cat(features).numpy(), predictions, knn_cm_path, k=5)
