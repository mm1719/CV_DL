...
class MultiCropTransform:
    ...
    """
    def __init__(self, global_transform, local_transform, num_global_crops=2, num_local_crops=4):
...
def get_transform(cfg, mode='train', two_crop=False, multi_crop=False):
    ...
    if mode == 'train':
        if multi_crop:
            return MultiCropTransform(global_transform, local_transform,
...
def get_datasets(cfg, mode='supcon'):
    ...
    train_transform = get_transform(cfg, mode='train',
    ...
    val_set = datasets.ImageFolder(root=os.path.join(data_dir, 'val'), transform=val_transform)
    test_set = ImageFolderWithPath(root=os.path.join(data_dir, 'test'), transform=test_transform)
