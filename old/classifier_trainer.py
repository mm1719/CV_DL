...
def train_classifier(cfg):
    ...
    for epoch in range(cfg['epochs_cls']):
        ...
        logger.info(f"[Epoch {epoch+1}/{cfg['epochs_cls']}] Train Loss: {avg_loss:.4f} Train Acc: {train_acc:.4f} Val Acc: {val_acc:.4f} Time: {time.time() - start_time:.1f}s")

        # Save best
        if val_acc > best_acc:
            best_acc = val_acc
            save_path = os.path.join(cfg['output_dir'], cfg['timestamp'], f"classifier_best_{cfg['timestamp']}.pth")
            torch.save(classifier.state_dict(), save_path)
            logger.info(f"[模型儲存] 儲存最佳分類器至: {save_path}")

            # 畫 confusion matrix
            cm_path = os.path.join(cfg['output_dir'], cfg['timestamp'], f"confusion_matrix_epoch{epoch+1}.png")
