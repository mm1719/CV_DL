...
def get_dataloaders(config):
    ...
    if getattr(config, "num_augmentations", 1) > 1:
        train_dataset = MultiAugDataset(config.train_dir, transform=train_transform, n_views=config.num_augmentations)
    else:
        ...
        def tta_transform():
            return transforms.Compose([
                ...
                transforms.Lambda(lambda crops: torch.stack([
                    transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                         std=[0.229, 0.224, 0.225])(transforms.ToTensor()(crop))
                    for crop in crops
                ]))
            ])
