...
def extract_features(encoder, dataloader, device, tta_transform, tta_times):
    ...
    for _, paths in tqdm(dataloader):
        ...
        for _ in range(tta_times):
            augmented_images = torch.stack([tta_transform(Image.open(p).convert("RGB")) for p in paths])
...
def plot_tsne_all(features, domains, save_path):
    ...
    for domain in ['train', 'val', 'test']:
        ...
        plt.scatter(tsne_result[idxs, 0], tsne_result[idxs, 1], s=10, alpha=0.6, label=domain, color=colors[domain])
...
def main(cfg, timestamp):
    ...
    train_features = extract_features(encoder, train_loader, device, tta_transform, tta_times)
    train_domains = ['train'] * train_features.size(0)

    print("[Step 2] 提取 Val 特徵...")
    val_features = extract_features(encoder, val_loader, device, tta_transform, tta_times)
    val_domains = ['val'] * val_features.size(0)

    print("[Step 3] 提取 Test 特徵...")
    test_features = extract_features(encoder, test_loader, device, tta_transform, tta_times)
