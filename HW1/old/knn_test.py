...
def extract_features(
    model, dataloader, device, tta_transform=None, tta_times=1, sample_paths=None
):
    features, paths = [], []
    index = 0
    with torch.no_grad():
...
def main(cfg, timestamp):
    ...
    val_features, _ = extract_features(
        encoder, val_loader, cfg["device"], tta_transform, tta_times, val_sample_paths
    )
    ...
    test_features, test_paths = extract_features(
        encoder, test_loader, cfg["device"], tta_transform, tta_times, test_sample_paths
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--timestamp", type=str, required=True, help="對應的訓練結果資料夾 timestamp"
    )
