...
def plot_tsne(features, labels, save_path=None):
    ...
    scatter = plt.scatter(
        reduced[:, 0], reduced[:, 1], c=labels, cmap="tab20", s=10, alpha=0.6
    )
...
def main():
    ...
    encoder_path = os.path.join(
        cfg["output_dir"], args.timestamp, f"supcon_encoder_{args.timestamp}.pth"
    )
    ...
    save_path = os.path.join(
        cfg["output_dir"], args.timestamp, f"tsne_supcon_{args.mode}.png"
    )
