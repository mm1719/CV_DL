...
def train_supcon(cfg):
    ...
    logger.info(
        f"[SupCon] 使用 {cfg['num_global_crops']} 張 global crop 和 {cfg['num_local_crops']} 張 local crop"
    )
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=cfg["epochs_supcon"]
    )
    ...
    for epoch in range(cfg["epochs_supcon"]):
        ...
        for images, labels in tqdm(train_loader, desc=f"SupCon Epoch {epoch+1}"):
            ...
            global_images = torch.cat(global_views, dim=0).to(
                cfg["device"]
            )  # (num_global_crops * B, C, H, W)
        ...
        logger.info(
            f"[Epoch {epoch+1}/{cfg['epochs_supcon']}] Loss: {avg_loss:.4f}  Time: {time.time() - start_time:.1f}s"
        )
